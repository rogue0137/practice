...
def BFSnumIslands(grid: List[List[str]]) -> int:
    ...
    for i in range(len_rows):
        for j in range(len_cols):
            # print(f'i: {i}, j:{j}')
            # print(f'grid at i,j: {grid[i][j]}')
            if grid[i][j] == '1':
                island_counter += 1
                grid[i][j] = 0 # set to 0 because it's been "visited"
                island_queue = []
                island_queue.append(i * len_rows + j)
                while len(island_queue) > 0:
                    ...
                    if above >= 0 and grid[above][col] == '1':
                        island_queue.append(above * len_cols + col)
                        grid[above][col] = 0
                    if below < len_rows and grid[below][col] == '1':
                        island_queue.append(below * len_cols + col)
                        grid[below][col] = 0
                    if left >= 0 and grid[row][left] == '1':
                        island_queue.append(row * len_cols + left)
                        grid[row][left] = 0
                    if right < len_cols and grid[row][right] == '1':
                        island_queue.append(row * len_cols + right)
                        grid[row][right] = 0
